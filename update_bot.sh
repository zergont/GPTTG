...
# Запускайте этот скрипт вручную **или** по команде /update (с флагом --no-restart).
#   --no-restart   — отложить рестарт бота через systemd‑run (нужно, когда скрипт
#                    вызывается самим ботом и нельзя убивать текущий процесс).
...
# ── Подготавливаем лог ────────────────────────────────────────────────
> "$LOG_FILE"   # очистка файла (POSIX‑совместимая)
...
log "✅  Обновление завершено. Версия $NEW_HASH"