...
sudo apt update
sudo apt install -y at git

# Запустить службу планировщика задач
...
- `at` (планировщик задач для автообновления)
- systemd (для управления сервисом)

> **Примечание:** Команда `at` и служба `atd` устанавливаются автоматически скриптом `update_bot.sh` при первом обновлении.
