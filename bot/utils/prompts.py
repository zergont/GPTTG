...
def reminder_tools_instr_long() -> str:
    return (
        ...
        "При необходимости отмены используй 'cancel_reminders' — по ids или по фразе из текста; выбирай консервативно, уточняй при сомнениях."
    )


def reminder_tools_instr_short() -> str:
    return " Для напоминаний используй schedule_reminder/schedule_reminders; для отмены — cancel_reminders."
